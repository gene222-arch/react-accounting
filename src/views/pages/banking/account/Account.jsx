...
const Vendor = ({ alert, account }) => 
{
    ...
    const columns = [
        ...
        { 
            title: 'Name', 
            field: 'name', 
            render: ({ id, name }) => <StyledNavLink to={ PATH.UPDATE_ACCOUNT.replace(':id', id)} text={ name } />
        },
        { 
            title: 'Number', 
            field: 'number', 
    ...
    const onLoadFetchAll = () => dispatch(ACCOUNT.getAccounts());

    const handleClickDestroy = () => {
        dispatch(ACCOUNT.destroyAccounts({ ids }));
        setIds([]);
    };
            ...
            <MaterialTable
                ...
                title={ 
                    <ActionButton 
                        classes={ classes } 
                        ids={ ids } 
                        handleClickRedirect = { () => history.push(PATH.CREATE_ACCOUNT) }
