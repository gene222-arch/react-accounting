...
import { findAsync } from '../../../../services/banking/bank.account.reconciliation';
    ...
    }) => 
{
    ...
    const handleClickFetchTransactionsByAccount = async (accountParam = null) => 
    {
        const { data, message, status } = await findByAccountAsync({ account_id: accountParam.id ?? bankAccountReconciliationState.account_id });
    ...
    const onLoadFetchReconciliationById = async () => 
    {
        ...
        if (status === 'success') 
        {
            const { created_at, updated_at, account: account_, ...reconciliation } = data;
            
            setBankAccountReconciliationState(reconciliation);
            setAccount(account_);
            handleClickFetchTransactionsByAccount(account_);
        }
    }
